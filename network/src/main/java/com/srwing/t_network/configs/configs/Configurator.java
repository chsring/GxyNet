package com.srwing.t_network.configs.configs;
...
import com.srwing.t_network.configs.filter.RespFilter;
...
//              .logStrategy(logStrategy) // (Optional) Changes the log strategy to print out. Default LogCat
                .tag("GXYLOG")   // (Optional) Custom tag for each log. Default PRETTY_LOGGER
                .build();
    ...
    public final Configurator withConverterFactory(Converter.Factory factory) {
        FOYO_CONFIGS.put(ConfigKeys.NET_FACTORY, factory);
        return this;
    }
